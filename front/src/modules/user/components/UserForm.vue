<template>
  <Form @submit="onSubmit" class="flex flex-col gap-4">
    ...
    <div class="flex flex-col gap-1.5">
      <label class="text-sm font-medium text-surface-700">CPF</label>
      <BaseInput
        :modelValue="cpf"
        @update:modelValue="val => emit('update:cpf', val)"
        placeholder="000.000.000-00"
        maxlength="14"
      />
      <small v-if="errors.cpf" class="text-red-500 text-xs">{{ errors.cpf[0] }}</small>
    </div>
...
const emit = defineEmits(['cancel', 'update:name', 'update:lastName', 'update:cpf', 'submit'])
